...
// Aktiviere CORS, damit der Server Anfragen von anderen Domains akzeptiert
app.use(cors({ origin: /http:\/\/localhost/ }));
app.options('*', cors());

// Erstelle eine Route, die auf '/events' reagiert
app.get('/events', (req, res) => {
  // Hole den Wert der query-Parameter 'city' aus der Anfrage
  ...
  // Ansonsten mache eine API-Anfrage an den SeatGeek-Service und hole alle Events für die angegebene Stadt
  axios
  .get(`https://api.seatgeek.com/2/events?aid=123&venue.city=${city}`)
  .then((response) => {
    // Überprüfe, ob es Events in der angegebenen Stadt gibt
